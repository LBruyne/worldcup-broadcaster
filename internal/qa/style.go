...
const styleLearnPrompt = `你在观察一个QQ群的说话风格。根据群聊记录总结这个群的语言风格画像：常用的梗和口癖、句子长短、表情/标点习惯、互怼方式、活跃话题。80字以内，写成"模仿指南"语气（如：句子短平快，爱用xx梗……）。然后另起一行，从群聊记录里挑3条最能代表群友说话方式的真实消息（不要选机器人"AAA世界杯稳定盈利"自己的发言），格式：例句：「消息1」「消息2」「消息3」。直接输出，不要解释。`
